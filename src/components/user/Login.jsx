...
function Login() {
  ...
  const handleSubmit = (e) => {
    ...
    if (username && password) {
      // Send username and password in the request body, not headers
      axios
        .post(USER_LOGIN_URL, {
        ...
        .then((res) => {
          // On success, store token and set user
          setMessage(res.data.message || "Logged in successfully");
          setUsername("");
          setPassword("");
          setUser(username);
          localStorage.setItem("token", res.data.access);  // Store the token
        })
        .catch((err) => {
  ...
  return (
    <section className="py-32 bg-fade-pink h-screen px-6">
      {user ? (
        ...
        <>
          <h1 className="text-center text-royal-green-900 font-bold text-xl">Login</h1>
          {message && (
          ...
          <form onSubmit={handleSubmit} className="mt-12 max-w-sm mx-auto w-full">
            <input
            ...
            />
            <div className="text-center mt-4">
              <button type="submit" className="px-6 py-3 text-md bg-royal-green-900 text-white rounded w-full">
                Login
              </button>
            </div>
          </form>
